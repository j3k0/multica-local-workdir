...
- **Tell the agent to ignore the block.** When a `# Task Settings` heading is present in the description (tracked by `task_block_seen`, independent of whether its values parsed), the wrapper appends a note to the agent's system prompt: the block is already-applied wrapper config, not task content. The note is folded into the **same** `--append-system-prompt` as the workspace CLAUDE.md — claude's `--append-system-prompt` is not reliably repeatable, so a second flag could clobber the first. The block can also be wrapped in an HTML comment (`<!-- … -->`, heading and keys on their own lines) to hide it from rendered markdown; the parser still matches it.
